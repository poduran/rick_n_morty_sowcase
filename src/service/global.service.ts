...
import { CharacterInfo, Result as CharRes } from 'src/models/character-info';
import { LocationInfo, Result as LocRes } from 'src/models/location-info';
import { EpisodeInfo, Result as EpRes } from 'src/models/episode-info';
...
export class GlobalService {
  ...
  getSingleCharacter(id: string): Observable<CharRes> {
    let endpoint = this.apiUrl + 'character/' + id;

    return this.http.get<CharRes>(endpoint);
  }
  ...
  getSingleLocation(id: string): Observable<LocRes> {
    let endpoint = this.apiUrl + 'location/' + id;

    return this.http.get<LocRes>(endpoint);
  }
  ...
  getSingleEpisode(id: string): Observable<EpRes> {
    let endpoint = this.apiUrl + 'episode/' + id;

    return this.http.get<EpRes>(endpoint);
  }
